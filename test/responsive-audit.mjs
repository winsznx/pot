...
const LONG_WORD = "POT_RESPONSIVE_AUDIT_UNBREAKABLE_ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
...
function mockHiroTransactions() {
  return {
    ...
    results: Array.from({ length: 18 }, (_, i) => ({
      ...
      contract_call: {
        contract_id: "SP31DP8F8CF2GXSZBHHHK5J6Y061744E1TNFGYWYV.pot",
        function_name: i % 3 === 0 ? `contribute-${LONG_WORD}` : "contribute",
      },
    })),
  };
}
...
async function seedWorstCaseStorage(context, chain) {
  await context.addInitScript(
    ({ chainKind, longWord }) => {
      window.localStorage.setItem("pot_chain_kind", chainKind);
      window.localStorage.setItem("pot_audit_long_word", longWord);
      window.localStorage.setItem("pot_highscore", "987654321987654321");
